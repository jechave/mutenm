...
Ddg_ent_act(wt, mut)          → dg_ent_act(mut) - dg_ent_act(wt)
```
...
| `dg_ent_act(prot, ideal, pdb_site_active, beta)` | `kmat_asite()`, `g_ent_mode()` |
...
| `enm_analysis.R` | Derived properties | `cn`, `wcn`, `dactive`, `msfi`, `msfn` |
